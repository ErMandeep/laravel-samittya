...
use App\Events\Frontend\Auth\OfflineUserLoggedOut;
...
class LoginController extends Controller
{
    ...
    public function showLoginPage()
    {
        ...
        return view('frontend.auth.offline_login');

    }

    public function offline_login(Request $request)
    {   
        ...
        if($validator->passes()){
            $user = Offline_student::where('student_id',$request->s_id)->first();
            $credentials = $request->only('student_id');
            if($user){
                $authSuccess = \Illuminate\Support\Facades\Auth::guard('offline')->loginUsingId($user->id) ;
            if ($authSuccess) {
                $request->session()->regenerate();
    ...
    public function offlinelogout(Request $request)
    {
         ...
         * Fire event, Log out user, Redirect
         */
        // event(new OfflineUserLoggedOut($request->user()));
