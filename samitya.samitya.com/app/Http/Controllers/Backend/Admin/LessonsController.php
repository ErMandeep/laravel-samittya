...
class LessonsController extends Controller
{
    ...
    if(auth()->user()->hasRole('administrator')){
             ...
             $display_duration = $request->display_duration1.' '.$request->display_duration2;
  
    $request->request->add(['display_duration' => $display_duration]);
    ...
    $request->request->add(['display_duration' => $display_duration]);
        ...
        $check_course = Course::where('course_id', '=', $request->course_id)->first();
        if ($check_course === null) {


           // user doesn't exist
           $existing_course_id = $request->course_id;

          // print_r($request->course_id);die;
        ...
        $oldcourse = Course::findOrFail($existing_course_id)->toArray();
        // $oldcourse->oldcourse->add(['teachers' => $current_user]);
        $oldcourse['teachers'] = $current_user;
       // echo "<pre>"; print_r($oldcourse); die;
        $course = Course::create($oldcourse);

        $course->reviewchanges = 0;
        $course->course_id = $existing_course_id;
        $course->published = 0;
        $course->save();

        $teachers = \Auth::user()->isAdmin() ? array_filter((array)$oldcourse->input('teachers')) : [\Auth::user()->id];

        $course->teachers()->sync($teachers);

        $last_inserted_id = $course->id;
        ...
        $request->request->add(['reviewlession' => '0']);

        // echo "<pre>"; print_r($request->all());die;
              $lesson = Lesson::create($request->except('downloadable_files', 'lesson_image')
            + ['position' => Lesson::where('course_id', $request->course_id)->max('position') + 1]);

              $lesson->course_id = $last_inserted_id;
              $lesson->oldcourse_id = $existing_course_id;
              $lesson->save();
        ...
        return redirect()->route('admin.lessons.index', ['course_id' => $request->course_id])->withFlashSuccess(__('alerts.backend.general.underreview'));  


        }else{
        $check_course_array = Course::where('course_id', '=', $request->course_id)->first()->toArray();

          $courseID = $check_course_array['id'];
          $existing_course_id = $check_course_array['course_id'];
  
            // echo "<pre>"; print_r($request->course_id);die;
            // echo "<pre>"; print_r($check_course_array['course_id']);die;
...
// ******************************************************************
    ...
    public function edit($id)
    {
        ...
        $part = preg_split('/\s+/', $lesson['display_duration']);
        $lesson->display_duration1 = $part[0];
        $lesson->display_duration2 = $part[1];
        ...
        return view('backend.lessons.edit', compact('lesson', 'courses', 'videos','preview'));
    }
  ...
  if ($check_course === null) {
       ...
       // echo "<pre>"; print_r($oldcourse); die;
        ...
        $request->request->add(['reviewlession' => '0']);
        $request->request->add(['lession_id' => $id]);
 ...
 $check_course_array = Course::where('course_id', '=', $request->course_id)->first()->toArray();
          $existing_course_id = $request->course_id;
        $last_inserted_id = $check_course_array['id'];

          // print_r($existing_course_id);die;
