...
// ************** course add by teacher
  ...
  if(!auth()->user()->hasRole('administrator')){
    ...
    $request->request->add(['teachers' => $current_user]);
    ...
    public function edit($id)
    {
        ...
        $part = preg_split('/\s+/', $course['display_duration']);
        $course->display_duration1 = $part[0];
        $course->display_duration2 = $part[1];
...
// echo "<pre>"; print_r($course);die;
        ...
        return view('backend.courses.edit', compact('course', 'teachers', 'categories','tags'));
    }
...
if(isset($request->reviewchanges)){
    ...
    echo "for teacher";
$request = $this->saveFiles($request);
        $oldcourse = Course::findOrFail($id);
if(!$request->free){
    $request->request->add(['free' => 0]);
}

        $request->request->add(['course_id' => $oldcourse->id]);
        $course = Course::create($request->all());
        $course->published = 0;
        $course->save();
        ...
        return redirect()->route('admin.courses.index')->withFlashSuccess(trans('alerts.backend.general.underreview'));
