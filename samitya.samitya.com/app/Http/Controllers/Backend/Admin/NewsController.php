...
class NewsController extends Controller
{
    ...
    public function index()
    {
        ...
        // die('dkfjkjd');
        // Show the page
        return view('backend.news.index', compact('pages'));
    ...
    public function getData(Request $request)
    {
        ...
        if (request('show_deleted') == 1) {
            ...
            //     return abort(401);
            // }
            $pages = News::onlyTrashed()->orderBy('created_at', 'desc')->get();

        } else {
            $pages = News::orderBy('created_at', 'desc')->get();
            ...
            ->addColumn('actions', function ($q) use ($has_view, $has_edit, $has_delete, $request) {
                ...
                if ($has_view) {
                    $view = view('backend.datatable.action-view')
                        ->with(['route' => route('admin.news.show', ['news' => $q->id])])->render();
                }
                if ($has_edit) {
                    $edit = view('backend.datatable.action-edit')
                        ->with(['route' => route('admin.news.edit', ['news' => $q->id])])
                        ->render();
                    $view .= $edit;
                }
                ...
                if ($has_delete) {
                    $delete = view('backend.datatable.action-delete')
                        ->with(['route' => route('admin.news.destroy', ['lesson' => $q->id])])
                        ->render();
                    $view .= $delete;
                }
...
// ***********************************************************************
 ...
 $lesson = News::create($request->except('page_image', 'lesson_image')
            + ['position' => News::where('user_id', $request->user_id)->max('position') + 1]);
...
// echo "<pre>";print_r($request->all());die;
        ...
        if ($request->position != "") {
            ...
            if (($request->media_type == 'youtube') || ($request->media_type == 'vimeo')) {
                ...
                $media = NewsMedia::where('url', $video_id)
                ...
                if (\Illuminate\Support\Facades\Request::hasFile('video_file')) {
                    ...
                    $media = NewsMedia::where('type', '=', $request->media_type)
                        ->where('model_type', '=', 'App\Models\Lesson')
                        ->where('model_id', '=', $lesson->id)
                        ->first();
                }
...
// ***********************************************************************
       $request = $this->saveall($request, 'page_image', News::class, $lesson);
        ...
        if ($lesson->id) {
            return redirect()->route('admin.news.index')->withFlashSuccess(__('alerts.backend.general.created'));
        } else {
            return redirect()->route('admin.news.index')->withFlashDanger(__('alerts.backend.general.error'));
    ...
    public function show($id)
    {
        ...
        //     return abort(401);
        // }
         $lesson = News::with('media')->findOrFail($id);

        $mediafiles = NewsMedia::where('model_id', '=', $lesson->id)->get();
        $page = News::findOrFail($id);
        return view('backend.news.show', compact('page', 'mediafiles'));
    ...
    public function edit($id)
    {
        ...
        return view('backend.news.edit', compact('mediafiles','preview' ,'page', 'category', 'tags'));
...
// **********************************************************
        ...
        return redirect()->route('admin.news.index')->withFlashSuccess(__('alerts.backend.general.updated'));
    ...
    public function destroy($id)
    {
        ...
        $page = News::findOrfail($id);
        $page->delete();
        return redirect()->route('admin.news.index')->withFlashSuccess(__('alerts.backend.general.deleted'));
    ...
    public function massDestroy(Request $request)
    {
        ...
        public function newsmediadelete(Request $request){

    
        $media_id =  $request->media_id;
        $media = NewsMedia::find($media_id);
        if($media){
            //Delete Related data
    ...
    public function restore($id)
    {
        ...
        $page = News::onlyTrashed()->findOrFail($id);
        $page->restore();

        return redirect()->route('admin.news.index')->withFlashSuccess(trans('alerts.backend.general.restored'));
    }
