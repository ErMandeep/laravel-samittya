...
class ReportController extends Controller
{
    public function getSalesReport()
    {
        ...
        $live_courses = LiveClasses::ofTeacher()->where('published','=',1)->pluck('id');
        ...
        $live_course_earnings = OrderItem::whereHas('order',function ($q){
            $q->where('status','=',1);
        })->where('item_type','=',LiveClasses::class)
            ->whereIn('item_id',$live_courses);

        $live_course_sales = $live_course_earnings->count();
        $live_course_earnings = $live_course_earnings->sum('price');

        $total_earnings = $course_earnings+$bundle_earnings+$live_course_earnings;
        $total_sales = $course_sales+$bundle_sales+$live_course_sales;

        // print_r($live_course_earnings);die;
    ...
    public function getliveCourseData(Request $request)
    {

        $courses = LiveClasses::ofTeacher()->where('published','=',1)->pluck('id');
