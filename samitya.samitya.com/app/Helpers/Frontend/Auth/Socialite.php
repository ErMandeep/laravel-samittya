...
class Socialite
{
    ...
    public function getSocialLinks()
    {
        ...
        if (config('services.facebook.active')) {
            $socialite_enable[] = "<a href='".route('frontend.auth.social.login', 'facebook')."' class='btn btn-sm btn-info text-white  p-1 px-2  m-1 my-3' id='facebook' ><i class='fab fa-facebook-f'></i> Login with facebook </a>";
        }


        if (config('services.google.active')) {
            $socialite_enable[] = "<a href='".route('frontend.auth.social.login', 'google')."' class='btn btn-sm btn-info text-white   p-1 px-2  m-1 my-3' id='google' ><i class='fab fa-google'></i> Login with google </a>";
        }
