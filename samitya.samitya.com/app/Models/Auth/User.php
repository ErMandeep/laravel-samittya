...
class User extends Authenticatable implements MessageableInterface
{
    ...
    {
        return $this->belongsToMany(LiveClasses::class, 'live_class_user');
    }
    public function live_classes_review()
    {
        return $this->belongsToMany(LiveClasses::class, 'live_class_user_review');
    }
    ...
    public function purchasedliveCourses()
    {
        $orders = Order::where('status','=',1)
            ->where('user_id','=',$this->id)
            ->pluck('id');
        $courses_id = OrderItem::whereIn('order_id',$orders)
            ->where('item_type','=',"App\Models\LiveClasses")
            ->where('subscribe','=','1')
            ->pluck('item_id');
        $courses = LiveClasses::where('published','=',1)
            ->whereIn('id',$courses_id)
            ->get();
        return $courses;
    }
