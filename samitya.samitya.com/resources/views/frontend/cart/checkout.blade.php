...
if(@$value['attributes']['free'] == 1){
    $rate = 'Free';
    
}else{
    $rate = $appCurrency['symbol'].' '.$value['attributes']['price'];
}

     if (in_array($course->id, $value)) {
@endphp  
...
@endphp   
   ...
   @php  
        ...
        $f = Cart::session(auth()->user()->id)->getContent()->toArray();
        foreach ($f as $value ) {
if(@$value['attributes']['duration'] == 0){
    $months = 'Unlimited';
    
}else{
    $months = $value['attributes']['duration'];
}
...
@endphp                                           
         ...
         @php }} @endphp  
                        ...
                        @if(count($courses) > 0)
                            @if((config('services.stripe.active') == 0) && (config('paypal.active') == 0) && (config('payment_offline_active') == 0) && (config('services.razorpay.active') == 0) )
...
@if($total == 0)
                                         ...
                                         <div class="razorpay_wrapper">
                                            ...
                                            <form action="{{route('cart.razorpay.free')}}" method="POST" >
                                                ...
                                                <button class="razorpay-payment-button">Pay now</button>            
                                            </form>
...
@else
                                         ...
                                         <div class="razorpay_wrapper">
                                            ...
                                            <form action="{{route('cart.razorpay.payment')}}" method="POST" >
                                                <input type="hidden" name="sub_dur" value="{{ encrypt($course->sub_duration) }}">
                                                <!-- Note that the amount is in paise = 50 INR -->
                                                <!--amount need to be in paisa-->
                                                <script src="https://checkout.razorpay.com/v1/checkout.js"
