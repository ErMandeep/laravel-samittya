...
<div class="sidebar">
    <nav class="sidebar-nav">
        <ul class="nav">
            ...
            @if(!$logged_in_user->can('offline_students_access'))
            <li class="nav-item">
...
@if ($logged_in_user->isAdmin())
                <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(2) == 'approveteacher' ? 'active' : '' }}"
                       href="{{ route('admin.auth.approve.index') }}">
                        <i class="nav-icon icon-folder-alt"></i>
                        <span class="title">Approve Teacher</span>
                    </a>
                </li>
 @endif
            @can('category_access')
            ...
            @endcan
            @if($logged_in_user->isAdmin() || Gate::allows('tags_access') )
                <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(2) == 'tags' ? 'active' : '' }}"
            ...
            @if($logged_in_user->isAdmin() || Gate::allows('live_class_access'))
            
                    <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/liveclasses*','user/schedule*']), 'open') }}">
                    <a class="nav-link nav-dropdown-toggle {{ active_class(Active::checkUriPattern('admin/*')) }}"
                       href="#">
                        <i class="nav-icon icon-puzzle"></i> Live Classes Management
                    </a>

                    <ul class="nav-dropdown-items">
                            <li class="nav-item ">
                                <a class="nav-link {{ $request->segment(2) == 'liveclasses' ? 'active' : '' }}"
                                   href="{{ route('admin.liveclasses.index') }}">
                                    <span class="title">Live Classes</span>
                                </a>
                            </li>
                           <!--  <li class="nav-item ">
			                    <a class="nav-link {{ $request->segment(2) == 'schedule' ? 'active' : '' }}"
			                       href="{{ route('admin.schedule.index') }}">
			                        <span class="title">Schedule</span>
			                    </a>
			                </li> -->
                            <li class="nav-item ">
                                <a class="nav-link {{ $request->segment(2) == 'reviewliveclasses' ? 'active' : '' }}"
                                   href="{{ route('admin.liveclasses.reviewindex') }}">
                                    <span class="title">Review Live Classes</span>
                                </a>
                            </li>
            ...
            @if($logged_in_user->hasRole('teacher') || $logged_in_user->isAdmin()  )
                ...
                <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/courses*','user/lessons*','user/tests*','user/questions*']), 'open') }}">
                    ...
                    <ul class="nav-dropdown-items">
                        ...
                        @if ($logged_in_user->isAdmin())
                            <li class="nav-item">
                                <a class="nav-link {{ $request->segment(2) == 'reviewchanges' ? 'active' : '' }}"
                                   href="{{ route('admin.reviewchanges.index') }}">
                                    <span class="title">@lang('menus.backend.sidebar.reviewchanges.title')</span>
                                </a>
                            </li>
                        @endif                        
            ...
            @if ($logged_in_user->isAdmin() || $logged_in_user->hasAnyPermission(['blog_access','page_access','reason_access']))
               ...
               <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(2) == 'gallery' ? 'active' : '' }}"
                       href="{{ route('admin.gallary.index') }}">
                        <i class="nav-icon icon-folder-alt"></i>
            ...
            <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/messages,user/messages_report,user/show_messages']), 'open') }}">
                <a class="nav-link nav-dropdown-toggle {{ active_class(Active::checkUriPattern('admin/*')) }}"
                       href="#">
                        <i class="nav-icon icon-envelope-open"></i> Message Management
                    ...
                    <li class="nav-item ">
                        <a class="nav-link {{ $request->segment(1) == 'messages' ? 'active' : '' }}"
                           href="{{ route('admin.messages') }}">
                            <span class="title">
                                @lang('menus.backend.sidebar.messages.title')
                            </span>
                        </a>
                    </li>
                    <li class="nav-item ">
                        <a class="nav-link {{ $request->segment(1) == 'messages_report' ? 'active' : '' }}"
                           href="{{ route('admin.messages.show_report') }}">
                            <span class="title">Reported Users</span>
                        </a>
                    </li>
                    <li class="nav-item ">
                        <a class="nav-link {{ $request->segment(1) == 'show_messages' ? 'active' : '' }}"
                           href="{{ route('admin.messages.show_messages') }}">
            ...
            @elseif(!auth()->user()->hasRole('offline users') && !auth()->user()->can('offline_students_access'))
            <li class="nav-item ">
                <a class="nav-link {{ $request->segment(1) == 'messages' ? 'active' : '' }}"
                   href="{{ route('admin.messages') }}">
            ...
            @endif
            @if ($logged_in_user->hasRole('teacher') || $logged_in_user->hasRole('live teacher'))
                <li class="nav-item ">
            ...
            @if ($logged_in_user->isAdmin() ) 
            <li class="nav-item nav-dropdown {{ active_class(Active::checkUriPattern(['user/states*','user/cities*','user/branches*','user/supervisor*','user/offline-student*']), 'open') }}">
                    <a class="nav-link nav-dropdown-toggle {{ active_class(Active::checkUriPattern('admin/*')) }}"
                       href="#">
                        <i class="nav-icon icon-pie-chart"></i>Offline Data Management

                    </a>
                    <ul class="nav-dropdown-items">
                        <li class="nav-item ">
                            <a class="nav-link {{ $request->segment(1) == 'states' ? 'active' : '' }}"
                               href="{{ route('admin.states.index') }}">States
                            </a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link {{ $request->segment(1) == 'cities' ? 'active' : '' }}"
                               href="{{ route('admin.cities.index') }}">Cities
                            </a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link {{ $request->segment(1) == 'branches' ? 'active' : '' }}"
                               href="{{ route('admin.branches.index') }}">Branches
                            </a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link {{ $request->segment(1) == 'supervisor' ? 'active' : '' }}"
                               href="{{ route('admin.supervisor.index') }}">Supervisors
                            </a>
                        </li>
                        <li class="nav-item ">
                            <a class="nav-link {{ $request->segment(1) == 'offline-student' ? 'active' : '' }}"
                               href="{{ route('admin.offline-student.index') }}">Students
                            </a>
                        </li>
                    </ul>
                </li>
            ...
            @can('offline_students_access')

                <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(1) == 'offline-student' ? 'active' : '' }}"
                               href="{{ route('admin.offline-student.index') }}">
                               <i class="nav-icon fa fa-user" aria-hidden="true"></i>Students
                            </a>
                </li>
            ...
            @if(!auth()->user()->hasRole('offline users'))
            <li class="nav-item ">
...
@elseif( auth()->check() &&  @$logged_in_user->hasRole('live teacher') )
                <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(2) == 'approveteacher' ? 'active' : '' }}"
                       href="{{ route('admin.approveteacher.upgrade') }}">
                        <i class="nav-icon icon-speech"></i> <span
                                class="title">Request For Regular Teacher</span>
                    </a>
                </li>
...
@elseif( auth()->check() &&  @$logged_in_user->hasRole('teacher') )
                <li class="nav-item ">
                    <a class="nav-link {{ $request->segment(2) == 'approveteacher' ? 'active' : '' }}"
                       href="{{ route('admin.approveteacher.upgrade') }}">
                        <i class="nav-icon icon-speech"></i> <span
                                class="title">Request For Live Teacher</span>
                    </a>
                </li>
@endif                
