...
@section('content')
    <div class="card">
        <div class="card-header">
            <h3 class="page-title float-left mb-0">Live Classes </h3>
            ...
            <div class="table-responsive">
                ...
                <div class="d-block">
                    <ul class="list-inline">
                        ...
                        |
                        <li class="list-inline-item">
                            <a href="{{ route('admin.liveclasses.index') }}?show_deleted=1"
                ...
                <table id="myTable" class="table table-bordered table-striped @can('live_class_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                    <thead>
                    <tr>
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.liveclasses.get_data')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.liveclasses.get_data',['show_deleted' => 1])}}';
            @endif

            @if(request('teacher_id') != "")
                route = '{{route('admin.liveclasses.get_data',['teacher_id' => request('teacher_id')])}}';
            @endif

            @if(request('cat_id') != "")
                route = '{{route('admin.liveclasses.get_data',['cat_id' => request('cat_id')])}}';
            @endif
