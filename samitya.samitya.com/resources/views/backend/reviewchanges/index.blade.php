        ...
        <div class="card-header">
            ...
            @can('course_create')
                <div class="float-right">
                    <a href="{{ route('admin.reviewchanges.create') }}"
            ...
            <div class="table-responsive">
                <div class="d-block">
                    <ul class="list-inline">
                        ...
                        |
                        <li class="list-inline-item">
                            <a href="{{ route('admin.reviewchanges.index') }}?show_deleted=1"
                ...
                <table id="myTable" class="table table-bordered table-striped @can('course_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                    ...
                    <tr>
                        ...
                        <th>@lang('labels.backend.courses.fields.email')</th>
...
@push('after-scripts')
    <script>
        ...
        $(document).ready(function () {
            var route = '{{route('admin.reviewchanges.get_data')}}';

            @if(request('show_deleted') == 1)
                route = '{{route('admin.reviewchanges.get_data',['show_deleted' => 1])}}';
            @endif

            @if(request('teacher_id') != "")
                route = '{{route('admin.reviewchanges.get_data',['teacher_id' => request('teacher_id')])}}';
            @endif

            @if(request('cat_id') != "")
                route = '{{route('admin.reviewchanges.get_data',['cat_id' => request('cat_id')])}}';
            @endif
            console.log(route);
            ...
            {{--@can('course_delete')--}}
            {{--@if(request('show_deleted') != 1)--}}
            {{--$('.actions').html('<a href="' + '{{ route('admin.reviewchanges.mass_destroy') }}' + '" class="btn btn-xs btn-danger js-delete-selected" style="margin-top:0.755em;margin-left: 20px;">Delete selected</a>');--}}
            {{--@endif--}}
            {{--@endcan--}}
        });
