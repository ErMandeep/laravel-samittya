    ...
    <div style="width:300pt; float:left;">
        ...
        <div class="panel panel-default">
            <div class="panel-body">
                ...
                @endif
                @if($contact_data["primary_address"]["status"] == 0 && $contact_data["secondary_address"]["status"] == 1)
                    <span>Address: {{$contact_data["secondary_address"]["value"]}} </span><br>
                @endif
                ...
                @endif
                 @if($contact_data["primary_phone"]["status"] == 0  && $contact_data["secondary_phone"]["status"] == 1)
                    {{--{{dd($contact_data["secondary_phone"]["value"])}}--}}
                    <span style="font-family: Helvetica, Arial, sans-serif;">Contact No.: {{ $contact_data["secondary_phone"]["value"]}}</span><br>
                @endif

                @if($contact_data["primary_email"]["status"] == 1)
                    <span> Email : {{$contact_data["primary_email"]["value"]}} </span><br>
                @endif
                @if($contact_data["primary_email"]["status"] == 0 && $contact_data["secondary_email"]["status"] == 1)
                    <span> Email : {{$contact_data["secondary_email"]["value"]}} </span><br>
                @endif
                    <span> Website : <a href="{{ url('/') }}">{{ url('/') }}</a> </span>
                @else
    ...
    <div style="clear:both; position:relative;">
        @if($invoice->notes)
                <div style="position:absolute; left:0pt; width:250pt;">
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                           {!! htmlspecialchars_decode(stripslashes($invoice->notes))  !!} 
                        </div>
                    </div>
                </div>
            @endif
